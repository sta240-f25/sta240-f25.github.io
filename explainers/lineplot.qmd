...
- This command is basicall6 evaluating the function at many points and connecting the dots. `n` specifies how many dots. Too few, and the picture would look all cattywampus:

```{r}
curve(x^2, from = -5, to = 5, n = 5)
```

If you want multiple lines on one graph, you can call `curve` multiple times, and include the `add` argument to all but the first call:
