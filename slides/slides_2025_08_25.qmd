...
library(tidyverse)
gs4_deauth()
d <- read_sheet("https://docs.google.com/spreadsheets/d/1qQCQPoooKDXOby9Z4cMgnktUwLLnhodz8R3wR1lThxw/edit?usp=sharing") |>
  rename(birthday = `What is your birthday?`,
         ...
         price = `As of this morning, what is the price (USD) of this product at Harris Teeter?`,
...
| ![](images/al.jpg){.c} | Liu, Aurora | Head TA    | TBD           |
| ![](images/lm.png){.c} | Ma, Liane   | TA         | TBD           |
...
ggplot(d, aes(x = price)) + 
  geom_histogram() + 
  labs(
    title = "How much does the ice cream cost?",
    x = "Price ($)"
  ) + 
...
::: callout-warning
## [We frequently forget and relearn this lesson.](https://polymarket.com)
...
## Sally Clark (1964 - 2007) {.scrollable}
...
::: incremental
- English solicitor;
- 1996: first son dies in bed at 11 weeks;
- 1998: second son dies in bed at 8 weeks;
- Clark arrested on suspicion of murder;
- Defense claims it was SIDS (sudden infant death syndrome).
:::
...
- Gave erroneous "expert" testimony that there was a "1 in 73,000,000" chance of two SIDS deaths in the same family;
    - first, that's not true;
    - second, so what?
- 1999: Clark is convicted. 
...
::: incremental
- Stats community loudly pans Meadow's testimony;
...
## Example: the mathematics of insurance (upgrade!)
...
- There are only probabilities associated with the possible states;
- Probability is fundamentally baked into the fabric of reality;
...
## We're building a cathedral! {.scrollable}

From the simplest building blocks, we will erect an impressive edifice of thought.
...
::: {.column width="55%"}
Ostensibly random behavior can nevertheless be quite patternful, in ways that we can actually penetrate with mathematics. 
