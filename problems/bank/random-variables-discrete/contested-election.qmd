...
a.  Based on your answers to the previous parts, what is the probability that the election result is reversed when the $k$ bad ballots are removed? Give a general formula for this probability, and then compute the probability for each of the real elections listed in @tbl-elections;
a.  The random variable $X$ that you derived in the first part can be approximated by $\text{Binom}(k,\,1/2)$. Explain why this approximation might be "good enough," and verify that it's not too bad by computing the *approximate* probability of reversal for the elections in @tbl-elections. Compare the approximate probabilities to the actual ones from the previous part;
a.  Let's say that we consider the probability of reversal "too big" if it is greater than 5%. Create a figure in `R` with the following features:
...
a.  Use the figure to comment on the match or lack thereof between our model and the decision that was actually made;
a.  How do you think a court should handle a legitimate election challenge? What are their responsibilities to the electorate, and what should they prioritize? Should they err on the side of upholding or voiding results? Should they think probabilistically? Should they consult this model, or one like it?
