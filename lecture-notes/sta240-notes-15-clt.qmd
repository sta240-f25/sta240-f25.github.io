...
plot_ks <- function(F1, F2, xrange = c(-5, 5), n = 2001) {
  ...
  plot(x, f1_vals, type = "l", lwd = 2, col = "blue",
       ylim = c(0,1), xlab = "x", ylab = "CDF", main = "CDFs with KS Distance")
...
F1 <- function(x) pexp(x, rate = 2)        # e.g. exponential
F2 <- function(x) punif(x, min = 0, max = 2)
plot_ks(F1, F2, xrange = c(0, 2))
...
- The distribution of $Z_n$ converges to *the same thing* no matter what the underlying $P$ is;
- That *same thing* is the standard bell curve.

Why should *any* of those things be true? The correct but vacuous answer is "because the proof says so." But apart from that, *why* is the central limit theorem true? The best answer I've encountered has to do with **entropy**, which is a measure of how random a variable is. It turns out the normal distribution is the maximum entropy distribution among all distributions with finite mean and variance, and so the central limit theorem is a monotone convergence theorem in the sense of entropy, and it says that the maximum is actually obtained in the limit[^1]. This result is due to a [paper](https://doi.org/10.1214/aop/1176992632) by Andrew Barron (shout out to a Rice undergrad alum). I doubt you can read that paper today, but a person's reach should exceed their grasp. Keep studying math, and one day you will.
