...
$\theta$ could be any real number, so suffice it to say, there are a lot of possible guesses to choose from. Which is best, and how do we measure that? Every possible value of $\theta$ gives us a different density curve. Here are four possible guesses:
...
The MLE may or may not be biased. It depends on the distribution family you are working with. Because the estimator is consistent, whatever bias there is will eventually go away as you collect more data. How fast will the bias go away? It depends. Welcome to statistical theory!
:::
